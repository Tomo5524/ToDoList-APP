...
const renderForm = (...ele) => {
  ...
  let cur_ele =
    ele[0].classList.contains("fa-edit") ||
    ele[0].classList.contains("edit-btn")
      ? "edit"
      : "add-btn";
  console.log("🚀 ~ file: form.js ~ line 17 ~ renderForm ~ cur_ele", cur_ele);
  ...
  start_end_prio_div.setAttribute(
    "class",
    "start-end-prio_div d-flex flex-wrap"
  );
  ...
  priority_div.setAttribute(
    "class",
    "priority-div d-flex px-3 col-sm-12 col-md-4"
  );
  ...
  if (cur_ele == "edit") {
    ...
    add_btn.addEventListener("click", (e) => {
      ...
      // console.log(e)
      e.preventDefault();
      const edit_task = Task.edit_todo(
        title_box.value.trim(),
        dscr_box.value.trim(),
        date_box.value,
        start.value,
        end.value,
    ...
    add_btn.addEventListener("click", (e) => {
      ...
      if (title_box.value.trim()) {
        console.log(
          Task.get_current_project(),
          "before added new task, check current project, form.js"
        );
        const newObj = Task.add_task(
          title_box.value.trim(),
          dscr_box.value.trim(),
          date_box.value,
          start.value,
        ...
        warning_modal.setAttribute(
          "class",
          "warning-modal bg-light border text-center no-display p-2"
        );

        const warning_text = document.createElement("p");
        warning_text.setAttribute("class", "warning-text");
        warning_text.innerHTML =
          "You need to add a todo's title before adding a todo";
